...
class ActionModule(ActionBase):
    ...
    def driver_func(self, directory_path):
        ...
        for file in files:
            ...
            output_list = gt.structured_config_to_topology_input(
                output_list, node_dict, data['diagram_groups'], current_dict={})
        ...
        rank_nodes_list = []
        for _, v in nodes_rank_dict.items():
            rank_nodes_list += v
        undefined_rank_nodes = list(set(rank_nodes_list) ^ set(global_node_list))
        gt.generate_topology(nodes_rank_dict, graph_dict,
                          output_list, undefined_rank_nodes)
