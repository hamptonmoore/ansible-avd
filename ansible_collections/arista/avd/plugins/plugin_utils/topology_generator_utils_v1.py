...
def structured_config_to_topology_input(output_list, node_dict, diagram_groups, current_dict=None):
    ...
    if diagram_groups:
        ...
        if not output_list:
            ...
            structured_config_to_topology_input(
                new_dict["groups"], node_dict, diagram_groups, new_dict)
        else:
            found = False
            for entry in output_list:
                if diagram_group_var == entry["name"]:
                    structured_config_to_topology_input(
                        entry["groups"], node_dict, diagram_groups, entry)
                    found = True

            if not found:
                new_dict = {"name": diagram_group_var,
                            "groups": [], "nodes": []}
                output_list.append(new_dict)
                structured_config_to_topology_input(
                    new_dict["groups"], node_dict, diagram_groups, new_dict)
   ...
   Returns:
    ...
    for data in input_data:
        ...
        with graph_obj.subgraph(name='cluster_child_' + str(data['name'])) as subgraph:
            ...
            if "nodes" in data and data["nodes"]:
                pod_node_list = [node['name']
                                 for node in data["nodes"] if node['name'] != '0']
                new_rank_dict = {'undefined': {}}
                new_node_list = [
                    undefined_rank_node for undefined_rank_node in undefined_rank_nodes
                    if undefined_rank_node in pod_node_list]
            ...
            if "groups" in data and data["groups"]:
                draw_nested_subgraphs(
                    data["groups"], rank_dict, subgraph, undefined_rank_nodes)
...
def create_graph_and_set_properties():
    ...
    """
    graph_obj = graphviz.Graph(name='parent', format='svg', filename='topology.gv',
                               graph_attr={
                                    'splines': 'line'
                               },
                               node_attr={
...
def generate_topology(rank_dict, node_neighbour_dict, output_list, undefined_rank_nodes):
    ...
    draw_nested_subgraphs(
        output_list, rank_dict, graph_obj, undefined_rank_nodes)
